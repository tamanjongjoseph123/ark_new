...
export default function KingdomLeadership() {
  ...
  // Fetch mentorship courses -> first course's modules -> augment with counts
  const loadMentorship = async () => {
    let mounted = true
    setLoadingCourses(true)
    try {
      let list = await listCourses("mentorship")
      if (!mounted) return
      if (!Array.isArray(list) || list.length === 0) {
        const allCourses = await listCourses(null)
        list = (allCourses || []).filter(
          (c) => typeof c?.category === "string" && c.category.toLowerCase() === "mentorship"
        )
      }
          ...
          <>
            {loadingCourses && (
              <View style={styles.loadingWrap}>
                ...
                <Text style={styles.loadingText}>Loading mentorship courses...</Text>
              </View>
            )}
            {!loadingCourses && courses.length === 0 && (
              <View style={styles.emptyWrap}>
                <Text style={styles.emptyTitle}>No mentorship courses found</Text>
